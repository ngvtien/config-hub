...
import { secureCredentialManager, HelmCredential } from './secure-credential-manager'
...
class HelmCredentialManager {
  ...
  async storeHelmCredential(config: HelmConfig): Promise<string> {
    try {
      const credentialId = config.id || secureCredentialManager.generateCredentialId('helm', config.registryUrl)
  ...
  async testHelmCredential(credentialId: string): Promise<{ success: boolean; error?: string }> {
    try {
      const credential = await secureCredentialManager.getCredential(credentialId) as HelmCredential
      if (!credential) {
  ...
  private async testOCIRegistry(credential: HelmCredential): Promise<{ success: boolean; error?: string }> {
    try {
      // For OCI registries, test by attempting to login
      let loginCommand = ''
      ...
      switch (credential.authType) {
        ...
        case 'token':
          ...
          // For token-based auth, username is often a fixed value like 'oauth2accesstoken' for GCR
  ...
  private async testHelmRepository(credential: HelmCredential): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const axiosConfig: any = {
        timeout: 10000,
        validateStatus: (status: number) => status < 500 // Accept 4xx as valid response
      }
        ...
        case 'cert':
          ...
          if (fs.existsSync(certFile) && fs.existsSync(keyFile)) {
            axiosConfig.httpsAgent = {
              ...
              rejectUnauthorized: !credential.insecureSkipTlsVerify
            }
          }
          break
      }
      ...
      const response = await axios.get(indexUrl, axiosConfig)
      
      if (response.status === 200 && response.data.includes('apiVersion')) {
        return { success: true }
  ...
  // Pull Helm chart using stored credentials
  async pullChart(credentialId: string, chartName: string, version?: string, destination?: string): Promise<{ success: boolean; error?: string }> {
    try {
      const credential = await secureCredentialManager.getCredential(credentialId) as HelmCredential
      if (!credential) {
        return { success: false, error: 'Credential not found' }
      ...
      let pullCommand = `helm pull "${chartName}"`
      if (version) pullCommand += ` --version "${version}"`
      if (destination) pullCommand += ` --destination "${destination}"`

      // For OCI registries, use different authentication
      if (credential.registryUrl.includes('oci://')) {
        // Ensure we're logged in first
        const loginResult = await this.testOCIRegistry(credential)
        if (!loginResult.success) {
          return { success: false, error: `OCI login failed: ${loginResult.error}` }
        }
      }
      ...
      if (output.includes('Successfully packaged chart') || fs.existsSync(path.join(destination || '.', `${chartName.split('/').pop()}-${version || 'latest'}.tgz`))) {
        return { success: true }
      } else {
        return { success: false, error: `Failed to pull chart: ${output}` }
      }
    } catch (error) {
      console.error('Failed to pull Helm chart:', error)
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
  }
  ...
  // Get Helm repositories using a credential
  async getRepositoriesForCredential(credentialId: string): Promise<HelmRepository[]> {
    try {
      const credential = await secureCredentialManager.getCredential(credentialId) as HelmCredential
      if (!credential) {
        return []
...
const helmCredentialManager = new HelmCredentialManager()

// IPC Handlers for Helm operations
export function setupHelmHandlers(): void {
  // Store Helm credentials
  ipcMain.handle('helm:store-credential', async (_, config: HelmConfig) => {
    try {
      const credentialId = await helmCredentialManager.storeHelmCredential(config)
  ...
  ipcMain.handle('helm:test-credential', async (_, credentialId: string) => {
    try {
      return await helmCredentialManager.testHelmCredential(credentialId)
    } catch (error) {
      console.error('Failed to test Helm credential:', error)
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
  ...
  ipcMain.handle('helm:list-credentials', async (_, environment?: string) => {
    try {
      const credentials = await secureCredentialManager.listCredentials('helm', environment)
  ...
  ipcMain.handle('helm:delete-credential', async (_, credentialId: string) => {
    try {
      await helmCredentialManager.cleanupCertificates(credentialId)
  ...
  ipcMain.handle('helm:add-repository', async (_, credentialId: string, repoName: string) => {
    try {
      return await helmCredentialManager.addHelmRepository(credentialId, repoName)
    } catch (error) {
      console.error('Failed to add Helm repository:', error)
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
  ...
  ipcMain.handle('helm:list-charts', async (_, credentialId: string, repoName?: string) => {
    try {
      return await helmCredentialManager.listCharts(credentialId, repoName)
    } catch (error) {
      console.error('Failed to list Helm charts:', error)
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
  ...
  // Pull Helm chart
  ipcMain.handle('helm:pull-chart', async (_, credentialId: string, chartName: string, version?: string, destination?: string) => {
    try {
      return await helmCredentialManager.pullChart(credentialId, chartName, version, destination)
    } catch (error) {
      console.error('Failed to pull Helm chart:', error)
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
  ...
  ipcMain.handle('helm:find-credentials-by-registry', async (_, registryUrl: string) => {
    try {
      const credentials = await secureCredentialManager.findCredentials({ type: 'helm', registryUrl })
      return { success: true, data: credentials }
    } catch (error) {
  ...
  // Get repositories for credential
  ipcMain.handle('helm:get-repositories', async (_, credentialId: string) => {
    try {
      const repositories = await helmCredentialManager.getRepositoriesForCredential(credentialId)
