# ArgoCD Service Account Setup Script (WSL Version)
# This script creates a service account for Config Hub with proper permissions
# Designed for Windows host with Kubernetes running in WSL
...
Write-Host "ArgoCD Service Account Setup (WSL)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Save to temp file in Windows
$tempFile = [System.IO.Path]::GetTempFileName()
$patchYaml | Out-File -FilePath $tempFile -Encoding UTF8 -NoNewline

# Convert Windows path to WSL path
$wslTempPath = "/tmp/argocd-patch-$([guid]::NewGuid().ToString()).yaml"

# Copy file to WSL and apply patch
try {
    # Copy file to WSL
    wsl bash -c "cat > $wslTempPath" < $tempFile
    
    # Apply the patch using microk8s kubectl
    $result = wsl microk8s kubectl patch configmap argocd-cm -n $Namespace --patch-file $wslTempPath 2>&1
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ Service account configuration added" -ForegroundColor Green
    } else {
        throw "kubectl patch failed: $result"
    }
    
    # Cleanup WSL temp file
    wsl rm -f $wslTempPath
} catch {
    Write-Host "✗ Failed to update ConfigMap" -ForegroundColor Red
    Write-Host "Error: $_" -ForegroundColor Red
    wsl rm -f $wslTempPath 2>$null
...
wsl microk8s kubectl rollout restart deployment argocd-server -n $Namespace | Out-Null
Write-Host "Waiting for rollout to complete..."
wsl microk8s kubectl rollout status deployment argocd-server -n $Namespace --timeout=60s | Out-Null
...
try {
    argocd account update-password --account $AccountName --new-password $Password 2>&1 | Out-Null
...
$wslRbacPath = "/tmp/argocd-rbac-$([guid]::NewGuid().ToString()).yaml"
...
$rbacYaml | Out-File -FilePath $rbacTempFile -Encoding UTF8 -NoNewline

try {
    # Copy file to WSL
    wsl bash -c "cat > $wslRbacPath" < $rbacTempFile
    
    # Apply RBAC patch
    $result = wsl microk8s kubectl patch configmap argocd-rbac-cm -n $Namespace --patch-file $wslRbacPath 2>&1
